...
async function main() {
    ...
    app.get('/', logMiddleware, async (req, res) => {
        ...
        const texts = await db.collection('text').find().toArray();
        const messages = texts.map((text) => {
            return text.message;
        });
        res.render(path.resolve(__dirname,'views/index.ejs'), { users: names,  texts: messages });
    });
    ...
    app.post('/api/user', express.json(), async (req, res) => {
        ...
        await db.collection('text').insertOne({ message: message });
